...
namespace SafeSpaces.Api.Controllers
{
    [ApiController]
    ...
    public class RecordingsController : ControllerBase
    {
        ...
        public IActionResult List()
        {
            var files = Directory.GetFiles(_storageDir)
                .Select(f => new
                {
                    ...
                    url = Url.ActionLink("Download", values: new { name = Path.GetFileName(f) })
                })
        ...
        public IActionResult Download(string name)
        {
            ...
            var mime = Path.GetExtension(name).Equals(".wav", StringComparison.OrdinalIgnoreCase)
                ? "audio/wav"
                : "audio/webm";
